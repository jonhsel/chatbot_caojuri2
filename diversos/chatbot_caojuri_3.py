...
def carrega_arquivo (tipo_arquivo, arquivo):
    ...
    if tipo_arquivo == 'PASTA':
    	with tempfile.NamedTemporaryFile(suffix=['.pdf','.csv'], delete=False, ) as temp:
            temp.write(arquivo.read())
            nome_temp = temp.name
    	documento = carrega_pasta(nome_temp)
...
def sidebar():
    ...
    with tabs_assistente[0]:
        ...
        if tipo_arquivo == 'PASTA':
            pasta = st.file_uploader('Carregue o arquivo do tipo .pdf', type=['.pdf', '.csv', '.txt'], accept_multiple_files=True)
